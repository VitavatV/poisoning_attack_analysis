...
def apply_poisoning(dataset, client_indices, poison_ratio, target_class, poison_label, seed=42):
    """
    สร้าง poison_map สำหรับเปลี่ยน Label
    target_class: คลาสที่จะถูกเปลี่ยน (Source)
    poison_label: คลาสปลายทาง (Destination)
    """
    random.seed(seed)
    np.random.seed(seed)
    
    poison_map = {} # เก็บ {global_index: new_label}
    poisoned_indices_list = []
    clean_indices_list = []

    # ดึง Targets ทั้งหมดเพื่อตรวจสอบ Class
    all_targets = np.array(dataset.targets)
    
    # แยก Index ของ Client ว่าตัวไหนเป็น Target Class บ้าง
    candidates = []
    others = []
    ...
    # สุ่มเลือกตัวที่จะ Poison
    num_poison = int(len(candidates) * poison_ratio)
    selected_poison = np.random.choice(candidates, num_poison, replace=False)
    
    # สร้าง Map และแยก List
    for idx in client_indices:
        if idx in selected_poison:
            poison_map[idx] = poison_label
            poisoned_indices_list.append(idx)
        else:
            clean_indices_list.append(idx)
...
def get_client_dataloader(dataset, client_indices, config, is_attacker=False):
    """
    สร้าง DataLoader สำหรับ Client 1 ราย
    จัดการเรื่อง Ordering (Good>Bad) ที่นี่
    """
    ...
    # 2. Ordering Step (Key Contribution)
    final_indices = []
    shuffle_batch = True # Default for standard training
    
    ordering = config.get('data_ordering', 'shuffle')
    
    if ordering == 'shuffle':
        # ผสมรวมกันแล้ว Shuffle ใน DataLoader ปกติ
        final_indices = clean_idxs + poison_idxs
        shuffle_batch = True 
        
    elif ordering == 'good_bad':
        # Clean มาก่อน Poison
        # Shuffle ภายในกลุ่มตัวเองก่อน เพื่อความเป็น Random
        random.shuffle(clean_idxs)
        random.shuffle(poison_idxs)
        final_indices = clean_idxs + poison_idxs
        shuffle_batch = False # ห้าม Shuffle ใน Loader อีก เพื่อรักษาลำดับ
    ...
    # หมายเหตุ: CIFAR/MNIST ปกติเป็น PIL Image ต้องแปลงเป็น Tensor
    # เราสามารถใส่ transform ที่นี่ หรือใส่ตอนโหลด Dataset หลักก็ได้
    local_dataset = FederatedDataset(
        dataset, 
        final_indices, 
    ...
    loader = DataLoader(
        ...
        shuffle=shuffle_batch, # สำคัญ: ถ้า Good>Bad ต้องเป็น False
        num_workers=0 # ใช้ 0 เพื่อความปลอดภัยในการ Debug
    )
