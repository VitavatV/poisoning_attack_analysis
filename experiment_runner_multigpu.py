...
def train_client_worker(args):
    ...
    Args:
        args: Tuple of (client_id, global_weights, config, train_ds_only, client_indices_subset, device_ids, use_data_parallel)
    
    Returns:
        Trained model weights (state_dict)
    """
    client_id, global_weights, config, train_ds_only, client_indices_subset, device_ids, use_data_parallel = args
    ...
    if use_data_parallel and len(device_ids) > 1:
        local_model = nn.DataParallel(local_model, device_ids=device_ids)
        # Load weights into the underlying module
        local_model.module.load_state_dict(global_weights)
    else:
...
def detect_available_gpus():
    """Detect and return list of available GPU device IDs"""
    ...
    print(f"\n{'='*60}")
    print(f"MULTI-GPU DATAPARALLEL MODE")
    print(f"Detected {num_gpus} GPU(s) - Using ALL GPUs simultaneously")
    print(f"This will pool VRAM across all GPUs for larger models/batches")
...
def run_single_experiment(config, seed, device_ids):
    """Run a single experiment using DataParallel across all GPUs
    
    Args:
        config: Experiment configuration
        seed: Random seed
        device_ids: List of GPU device IDs to use (empty for CPU)
    """
    # Determine if we're using DataParallel
    use_data_parallel = len(device_ids) > 1
    ...
    if device_ids:
        ...
        if use_data_parallel:
            gpu_label = f"GPUs {device_ids} (DataParallel)"
        else:
            gpu_label = f"GPU {device_ids[0]}"
    else:
    ...
    # Set num_parallel_workers to 5 (as in original)
    ...
    print(f"\n>>> RUNNING Seed: {seed} on {gpu_label}")
    if use_data_parallel:
        print(f">>> DataParallel enabled: Model will be split across {len(device_ids)} GPUs")
        print(f">>> This pools VRAM: ~{len(device_ids)}x effective memory")
    print("\n" + "="*60)
    ...
    # Wrap with DataParallel if using multiple GPUs
    if use_data_parallel:
        print(f">>> Wrapping global model with DataParallel across GPUs: {device_ids}")
        global_model = nn.DataParallel(global_model, device_ids=device_ids)
        global_weights = global_model.module.state_dict()
    else:
    ...
    for round_idx in range(config['global_rounds']):
        ...
        worker_args = [
            (client_id, global_weights, config, train_ds_only, client_indices_subset, device_ids, use_data_parallel)
        ...
        if use_data_parallel:
            global_model.module.load_state_dict(global_weights)
        else:
            global_model.load_state_dict(global_weights)
        
        # --- Validation & Early Stopping ---
        val_loss, val_acc = evaluate_model(global_model, val_loader, primary_device)
        ...
        if early_stopper.early_stop:
            ...
            if use_data_parallel:
                global_model.module.load_state_dict(early_stopper.get_best_weights())
            else:
                global_model.load_state_dict(early_stopper.get_best_weights())
            break
            
    # 5. Final Test
    test_loss, test_acc = evaluate_model(global_model, test_loader, primary_device)
    print(f"FINAL RESULT: Test Acc: {test_acc:.4f}")
    
    # Get model for saving (unwrap if DataParallel)
    model_to_save = global_model.module if use_data_parallel else global_model
    num_params = sum(p.numel() for p in model_to_save.parameters())
    
    return test_acc, test_loss, best_acc, best_loss, best_epoch, num_params, model_to_save
...
def main():
    ...
    if not available_gpus:
        ...
        print(f"Will use DataParallel across {len(device_ids)} GPUs for VRAM pooling\n")
    
    # Support command-line config path
    if len(sys.argv) > 1:
        config_path_list = [sys.argv[1]]
    else:
        # Default: Process all config files
        config_path_list = [
            ...
            'configs/config_exp3_mnist_B.yaml',
            'configs/config_exp3_mnist_C.yaml',
            'configs/config_exp3_cifar10.yaml',
    ...
    for config_path in config_path_list:
        ...
        if device_ids:
            logging.info(f"Using DataParallel with GPUs: {device_ids}")
        else:
        ...
        for phase_name, phase_cfg in phases:
            ...
            for i_exp, exp in enumerate(exp_list):
                ...
                print(f"\n{'='*60}")
                print(f"Starting Experiment {i_exp+1}")
                print(f"Phase: {phase_name}")
                print(f"Dataset: {exp['dataset']}, Width: {exp['width_factor']}, Depth: {exp['depth']}")
                print(f"{'='*60}")
