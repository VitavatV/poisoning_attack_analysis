...
ORIGINAL_B = config.get("ORIGINAL_B", 0.3)
...
configs = [
    ("ML101_linear_CL_batch", N_PER_CLASS * N_CLASSES),
    # ("ML101_linear_CL_minibatch", 8),
    # ("ML101_linear_CL_SGD", 1),
]

for NAME_SAVE_PATH, BATCH_SIZE in configs:
    SAVE_MAIN_PATH = os.path.join("results",NAME_SAVE_PATH)
    ...
    for client_id in range(N_CLIENTS):
        SAVE_PATH = os.path.join(SAVE_MAIN_PATH, f"client_{client_id}")
        # Remove existing directory if it exists
        if os.path.exists(SAVE_PATH):
            shutil.rmtree(SAVE_PATH)
        if not os.path.exists(SAVE_PATH):
            os.makedirs(SAVE_PATH)
        X = np.load(os.path.join(DATA_PATH, f"ML100_data_X_client{client_id}.npy"))
        y = np.load(os.path.join(DATA_PATH, f"ML100_data_y_client{client_id}.npy"))
        ...
        model = SimpleLinear()
        with torch.no_grad():
            model.linear.weight[:] = torch.tensor([[0.5, 0.9]])
            model.linear.bias[:] = torch.tensor([1.0])
        ...
        for i, rec in enumerate(records):
            ...
            plt.tight_layout()
            save_name_path = os.path.join(SAVE_PATH, f'epoch_{rec["epoch"]:02d}_batch_{rec["batch"]:02d}.png')
            plt.savefig(save_name_path)
            # save_name_path = os.path.join(SAVE_all_PATH, f'{NAME_SAVE_PATH}_lastest.png')
            # plt.savefig(save_name_path)
            plt.close()
        ...
        save_name_path = os.path.join(SAVE_PATH, f'{NAME_SAVE_PATH}.csv')
